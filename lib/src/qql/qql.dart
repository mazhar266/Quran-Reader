// Dart FFI binding for the QQL native library.
//
// Vendored from QQ-Lang (bindings/dart/qql.dart) with one change: the default
// library path is resolved per Flutter target rather than assuming a file next
// to the executable. Keep the C signatures in sync with QQ-Lang's include/qql.h.
//
...
class _QqlLib {
  _QqlLib(String? path) : this._(_load(path));
  ...
  static DynamicLibrary _load(String? path) {
    ...
    // iOS links qql as a static library into the app binary, so the symbols
    // are already in the process — there is nothing to dlopen.
    if (Platform.isIOS || Platform.isMacOS) return DynamicLibrary.process();
    // Android finds it in jniLibs/<abi>/; Linux in the bundle's lib/ dir,
    // which the runner's rpath ($ORIGIN/lib) already covers.
    if (Platform.isWindows) return DynamicLibrary.open('qql.dll');
    return DynamicLibrary.open('libqql.so');
  }
}
